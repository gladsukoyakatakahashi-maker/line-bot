...
## 対応できる主な症状
首痛、肩こり、肩の痛み、腰痛、ぎっくり腰、膝痛、股関節痛、足首痛、坐骨神経痛、ストレートネック、寝違え、むくみ・冷え、頭痛、産後の骨盤矯正、交通事故の治療、スポーツ外傷、リハビリ など
    ...
    try {
      const completion = await groq.chat.completions.create({
        ...
        messages: [
          ...
          ...history,                                   // ← 過去の会話を全部渡す
          { role: 'user', content: userMessage },       // ← 今回のメッセージ
        ],
        max_tokens: 600,
        temperature: 0.7,
      });
...
async function handleEvent(event) {
  ...
  // ── AI相談モード中は何を送ってもAIが受け取る ──
  // ただし予約ボタン（新規予約・来院中の予約）は除外して予約フローへ
