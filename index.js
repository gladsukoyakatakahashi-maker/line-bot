...
## 重要な注意事項
- 医療診断・処方は行わない
- 「すぐ病院へ」が必要な緊急症状（激しい胸痛・呼吸困難・麻痺など）の場合は迷わず救急を勧める
- 料金の詳細は「院にお問い合わせください」と案内する（料金情報は準備中）
...
async function handleAiChat(userId, replyToken, userMessage, session) {
  ...
  await setSession(userId, { ...session, mode: 'ai_chat', aiHistory: history });
...
async function handleBooking(userId, replyToken, text, session) {
  ...
  if (step === 2) {
    ...
    await setSession(userId, { ...session, selectedDate: matched.toISOString(), allSlots: slots, slotsPage: 0, step: 3 });
    return replyMessages(replyToken, [{
      type: 'template',
      altText: '時間を選んでください',
      template: { type: 'buttons', text: `${formatDate(matched)}\nご希望の時間をお選びください（1/3）`, actions },
    }]);
  }
