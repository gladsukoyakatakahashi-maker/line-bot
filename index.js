...
async function handleBooking(userId, replyToken, text, session) {
  ...
  if (step === 4) {
    ...
    await setSession(userId, Object.assign({}, session, { phone: text, step: 5 }));
    return replyText(replyToken,
      '🩺 気になる症状を番号で入力してください（複数可・カンマ区切り）\n\n' +
...
async function handleEvent(event) {
  ...
  if (text === '新規予約') {
    ...
    const newSession = { mode: 'booking', bookingType: 'new', step: 1 };
    await setSession(userId, newSession);
    return handleBooking(userId, replyToken, text, newSession);
  }
  ...
  if (text === '来院中の予約') {
    ...
    const newSession = { mode: 'booking', bookingType: 'returning', step: 1 };
    await setSession(userId, newSession);
    return handleBooking(userId, replyToken, text, newSession);
  }
